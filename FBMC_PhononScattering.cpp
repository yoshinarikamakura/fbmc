...
void FBMC::constructPhononScatteringTables(std::string param_filename,
    ...
    hwmax_ = phonon_.getMaximumEnergyPerBand();
    const int NETA = phonon_.getNumberOfBands();

    ENERGY_GEOMETRIC_RATIO_
        = pow(emax / MINIMUM_ENERGY_FOR_SCATTERING_RATE_TABLE_,
              1.0 / SIZE_OF_SCATTERING_RATE_TABLE_);
    BETA_ = ELEMENTARY_CHARGE / (BOLTZMANN * temperature);
    FACTOR_PHONON_SCATTERING_RATE_ = 0.5 * factor_dos
                                     * k_unit * k_unit * M_PI * HBAR
                                     / DENSITY_;
    ...
    phonon_absorption_scattering_rate_table.resize(
        SIZE_OF_SCATTERING_RATE_TABLE_);
    phonon_emission_scattering_rate_table.resize(
        SIZE_OF_SCATTERING_RATE_TABLE_);
    for (int isc = 0; isc < SIZE_OF_SCATTERING_RATE_TABLE_; ++isc) {
        phonon_absorption_scattering_rate_table[isc].resize(NETA, 0.0);
        phonon_emission_scattering_rate_table[isc].resize(NETA, 0.0);
    }
...
void FBMC::loadMaterialParameter(std::string param_filename) {
    std::ifstream fin_param(TABLE_DIRECTORY_NAME_ + param_filename,
                            std::ios::in);
    if (!fin_param) {
    ...
    std::string name;
    double value;
    std::set<std::string> loaded_params;
    ...
    for (const auto& items : param_map) {
        if (loaded_params.find(items.first) == loaded_params.end()) {
            cerr << "# Missing material parameter: " << items.first << endl;
            exit(EXIT_FAILURE);
        }
    }
    fin_param.close();
...
void FBMC::loadPhononScatteringRate(std::string band_filename) {
    std::string phonon_scattering_rate_table_filename
        = TABLE_DIRECTORY_NAME_ + "Phonon_scattering_rate_" + band_filename;
    std::ifstream fin_ephrate(phonon_scattering_rate_table_filename,
                              std::ios::in);
    const int NETA = phonon_.getNumberOfBands();

    if (fin_ephrate.is_open()) {
        cout << "# File: " + phonon_scattering_rate_table_filename
                           + " is found.\n";
        ...
        cout << "# Now, making phonon scattering rate table."
             << " Please wait for a while.\n";

        makePhononScatteringRateTable();
        std::ofstream fout_ephrate(phonon_scattering_rate_table_filename,
                                   std::ios::out);
        for (int isc = 0; isc < SIZE_OF_SCATTERING_RATE_TABLE_; ++isc) {
            fout_ephrate << isc << endl;
            for (int eta = 0; eta < NETA; ++eta) {
                fout_ephrate
                    << phonon_absorption_scattering_rate_table[isc][eta]
                    << ' '
                    << phonon_emission_scattering_rate_table[isc][eta]
                    << endl;
            }
        }
        fout_ephrate.close();
...
State FBMC::selectStateAfterPhononScattering(const int eta,
    ...
    if (isAbsorption) {  // Phonon Absorption
        ...
        ief_max = static_cast<int>((initial_energy + hwmax_[eta])
                                   / DELTAE_MINMAX_);
    } else {  // Phonon Emission
        ief_min = static_cast<int>((initial_energy - hwmax_[eta])
                                   / DELTAE_MINMAX_);
    ...
    if (isAbsorption) {  // Phonon absorption
        ...
        ief_dosmax_max = static_cast<int>((initial_energy + hwmax_[eta])
                                          / DELTAE_DOSMAX_);
    } else {  // Phonon emission
        ief_dosmax_min = static_cast<int>((initial_energy - hwmax_[eta])
                                          / DELTAE_DOSMAX_);
        if (ief_dosmax_min < 0) {
            ief_dosmax_min = 0;
        }
    ...
    double me_max = isAbsorption
                    ? matrix_element_max[eta][1]   // Absorption
                    : matrix_element_max[eta][0];  // Emission
    ...
    int it = 0, ib = 0;
    const double sign = isAbsorption ? 1.0 : -1.0;
...
//        double sign = isAbsorption ? 1.0 : -1.0;
        final_energy = initial_energy
                       + sign * phonon_.getEnergy(State{vq, eta});
        ...
        double me = getToyMatrixElementForSilicon(State{vq, eta},
                                                  isAbsorption,
                                                  initial_energy);

        isSuccess = (dos * me > dosmax_max * me_max * urand(mt_))
                    ? true : false;
        loop += 1;
    ...
    if (!isSuccess) {
        ...
        cerr << "# ki = "
             << initial_state.k.x << ' '
             << initial_state.k.y << ' '
             << initial_state.k.z << endl;
        // exit(EXIT_FAILURE);
        return initial_state;
    }

    State final_state = carrier_.getStateInTetrahedron(final_energy,
                                                       it,
                                                       ib,
                                                       initial_state.r,
...
double FBMC::calculatePhononScatteringRate(const int eta,
                                           const bool isAbsorption,
                                           const State initial_state) {
    // Initial State Energy (eV)
    const double initial_energy = carrier_.getEnergy(initial_state);
    ...
    for (int ie1 = ie1_min; ie1 < ie1_max+1; ++ie1) {
        ...
        for (int ie41 = ie41_min; ie41 < NE41_MINMAX_; ++ie41) {
            candidates.insert(candidates.end(),
                              minmax_[ie1][ie41].begin(),
                              minmax_[ie1][ie41].end());
        }
    }
    ...
    return sum * FACTOR_PHONON_SCATTERING_RATE_;
}
...
void FBMC::makePhononScatteringRateTable(void) {
    ...
    for (int isc = 0; isc < SIZE_OF_SCATTERING_RATE_TABLE_; ++isc) {
        double initial_energy = MINIMUM_ENERGY_FOR_SCATTERING_RATE_TABLE_
                                * pow(ENERGY_GEOMETRIC_RATIO_, isc);

        for (int i = 0; i < NUMBER_OF_SAMPLING_FOR_PH_RATE_TABLE_; ++i) {
            // Randomly select an initial state
            State initial_state =
                selectStateOnIsoEnergySurface(initial_energy,
                                              Vector3(0.0, 0.0, 0.0),
                                              -1.0);
            for (int eta = 0; eta < NETA; ++eta) {
                phonon_absorption_scattering_rate_table[isc][eta]
                += calculatePhononScatteringRate(eta, true, initial_state);
                phonon_emission_scattering_rate_table[isc][eta]
                += calculatePhononScatteringRate(eta, false, initial_state);
            }
        }
    }
    ...
    for (int isc = 0; isc < SIZE_OF_SCATTERING_RATE_TABLE_; ++isc) {
        for (int eta = 0; eta < NETA; ++eta) {
            phonon_absorption_scattering_rate_table[isc][eta]
            /= NUMBER_OF_SAMPLING_FOR_PH_RATE_TABLE_;
            phonon_emission_scattering_rate_table[isc][eta]
            /= NUMBER_OF_SAMPLING_FOR_PH_RATE_TABLE_;
        }
    }
...
double FBMC::getPhononScatteringRate(const int eta,
                                     const bool isAbsorption,
                                     const State initial_state) {
    const double initial_energy = carrier_.getEnergy(initial_state);
    const int isc =
         static_cast<int>(log(initial_energy
                          / MINIMUM_ENERGY_FOR_SCATTERING_RATE_TABLE_)
                          / log(ENERGY_GEOMETRIC_RATIO_));
    ...
    if (isc < 0) {
        ...
        double e0 = MINIMUM_ENERGY_FOR_SCATTERING_RATE_TABLE_
                    * pow(ENERGY_GEOMETRIC_RATIO_, isc);
        double e1 = e0 * ENERGY_GEOMETRIC_RATIO_;
...
std::vector<std::vector<double>> FBMC::getMaxToyMatrixElement(
                                           const double eimax) {
    constexpr int NETA = 6;  // for getToyMatrixElementForSilicon()
    std::vector<std::vector<double>> memax;
    ...
    for (int eta = 0; eta < NETA; ++eta) {
        // Emission
        memax[eta][0] = getToyMatrixElementForSilicon(State{vqmax, eta},
                                                      false,
                                                      eimax);
        // Absorption
        memax[eta][1] = getToyMatrixElementForSilicon(State{vqmax, eta},
                                                      true,
                                                      eimax);
    }
...
double FBMC::getToyMatrixElementForSilicon(State phonon_state,
                                           const bool isAbsorption,
                                           const double carrier_energy) {
    const double hw = phonon_.getEnergy(phonon_state);
    double matrix_element = 0.0;

    if (hw > 1.0e-4) {
        double bose = 1.0 / (exp(hw * BETA_) - 1.0);
        ...
        switch (phonon_state.n) {
            case 0:  // LO
                dp2 = (ADKOP_ + BDKOP_ * carrier_energy) / (k_unit * k_unit);
                break;
            case 1:  // TO
                dp2 = (ADKOP_ + BDKOP_ * carrier_energy) / (k_unit * k_unit);
                break;
            case 2:  // TO
                ...
                break;
            case 3:  // LA
                dp2 = (ADAC_ + BDAC_ * carrier_energy) * q2;
                break;
            case 4:  // TA
                dp2 = (ADAC_ + BDAC_ * carrier_energy) * q2;
                break;
